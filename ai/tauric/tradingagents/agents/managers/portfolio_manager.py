...
def create_portfolio_manager(llm):
    ...
    def portfolio_manager_node(state) -> dict:
        ...
        if len(history) > 12000:
            agg = risk_debate_state.get("aggressive_history", "")
            cons = risk_debate_state.get("conservative_history", "")
            neut = risk_debate_state.get("neutral_history", "")
            eval_history = (
                f"### Aggressive Analyst (Key Points):\n{agg[-4000:]}\n\n"
                f"### Conservative Analyst (Key Points):\n{cons[-4000:]}\n\n"
                f"### Neutral Analyst (Key Points):\n{neut[-4000:]}"
            )
        else:
